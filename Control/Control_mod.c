...
static void PID_Postion_Cal(PID_Typedef * PID,float target,float measure,int32_t dertT)
{
	...
	if(flightModes.FLIGHT_ENABLED && offLandFlag){
...
void CtrlAttiAng(void)
{
		...
		if(altCtrlMode==MANUAL){
			angTarget[ROLL]=(float)(RCData.roll);
			angTarget[PITCH]=(float)(RCData.pitch);
		}else{
...
void CtrlAttiRate(void)
{
	...
	yawRateTarget=-(float)RCData.yaw;
...
#define TILT_MAX 					(PR_MAX_ANGLE * M_PI_F / 180.0 )
...
//Battery voltage
float estHoverThrust(void){
	float hoverHru;
	GetBattVolt();
	if(Battery.voltage > 4.05){
		hoverHru = -0.25f;
	}
	else if(Battery.voltage > 3.90){
		hoverHru = -0.40f;
	}
	else if(Battery.voltage > 3.80){
		hoverHru = -0.45f;
	}
	else if(Battery.voltage > 3.70){
		hoverHru = -0.50f;
	}
	else{
		hoverHru = -0.55f;
...
void CtrlAlti(void)
{
	...
	//only in climb rate mode and landind mode. now we don't work on manual mode
	//手动模式不使用该高度控制算法
	if(MANUAL == altCtrlMode || !flightModes.FLIGHT_ENABLED){
		return;
	}
	...
	manThr=RCData.throttle / 1000.0f;
	...
	satXY=0;
	satZ=0;
	thrustXYSp[0]= sinf(RCData.roll * TORAD) ;//目标角度转加速度
	thrustXYSp[1]= sinf(RCData.pitch * TORAD) ; 	//归一化
	thrustXYSpLen= sqrtf(thrustXYSp[0] * thrustXYSp[0] + thrustXYSp[1] * thrustXYSp[1]);
	//limit tilt max
	if(thrustXYSpLen >0.01f )
	{
		thrustXYMax=-thrustZSp * tanf(PR_MAX_ANGLE * TORAD);
	...
	rollSp= asinf(thrustXYSp[0]) * TODEG;
	pitchSp = asinf(thrustXYSp[1]) * TODEG;				
...
void CtrlMotor(void)
{
		float  cosTilt = imu.accb[2] / INS_G_VAL;
	
		if(altCtrlMode==MANUAL)
		{
			DIF_ACC.Z =  imu.accb[2] - INS_G_VAL;
			Thro = RCData.throttle;
		...
		MotorsMix(Motor, Roll, Pitch, Yaw);
	
   	if(flightModes.FLIGHT_ENABLED) 
			MotorsPWMFlash(Motor[0],Motor[1],Motor[2],Motor[3]);   
		else                  
