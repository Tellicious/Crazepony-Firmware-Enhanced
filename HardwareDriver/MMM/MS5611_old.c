...
void MS561101BA_readPROM(void) {
	...
	for (i=0;i<MS561101BA_PROM_REG_COUNT;i++) {
			/*I2C_read (MS5611_ADDR, MS561101BA_PROM_BASE_ADDR + (i * MS561101BA_PROM_REG_SIZE), buf, 2);
			PROM_C[i] = ((uint16_t)buf[1] << 8) | buf[0];*/
			IIC_Start();
...
uint32_t MS561101BA_getConversion(void) 
{
	...
	u8 temp[3];
	//I2C_read (MS5611_ADDR, MS5611_ADDR+1, temp, 3);
